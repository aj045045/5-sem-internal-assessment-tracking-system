...
def download_keys():
    create_keys()
    with zipfile.ZipFile('uploads/keys.zip', 'w') as zips:
        zips.write('uploads/private_key.pem')
        zips.write('uploads/public_key.pem')
    response = send_file('uploads/keys.zip', as_attachment=True)
...
def decrypt_file(file_path, private_key):
    ...
    output_file_path = file_path[:-9] + "-decrypt.docx"  # Remove the ".enc" extension and add "-decrypt.docx"

...
def update_semester(semester_id):
    ...
    semester = Controller.Semester()
    path = request.form.get('path')
    print(f'Path {path} and semester {semester_id} and subject {subject_plan.filename}')
...
def add_subject():
    ...
    if subject_plan is not None and subject_plan.filename != '':
        ...
        subject_location = ''
    subject.add_subject(subject_name,subject_type,code,credit,subject_location,semester_id,faculty_id)
    Controller.Database.collection('semester').update_one({'_id': ObjectId(semester_id) },{'$inc': {'number_of_subject': 1}})
    return redirect(path)
